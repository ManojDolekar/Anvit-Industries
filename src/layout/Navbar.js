...
function Navbar() {
    ...
    const navItems = [
        ...
        { id: 'about', label: 'About', onClick: () => handleScrollNav('about') },
        { id: 'contact', label: 'Contact', onClick: () => handleScrollNav('contact') }
    ];

    return (
        <>
            <nav className={`w-full z-50 transition-all duration-500 ease-in-out ${
                isOnTop ? 'fixed bg-transparent'  : !isScrolling 
                    ? ' fixed top-0 left-0 bg-[#ececec] backdrop-blur-md shadow-xl border-b border-blue-200/50'
                    : 'hidden'
            }`}>
                {/* Animated top border */}
                {/* <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-b    lue-600 via-cyan-500 to-blue-600"></div> */}
                    ...
                    <div className="flex justify-between items-center h-16 lg:h-20">
                        ...
                        <div className="flex items-center space-x-2 sm:space-x-3 group cursor-pointer">
                            <div className="relative">
                                <div className="absolute inset-0 bg-gradient-to-r from-blue-100 to-cyan-100 rounded-lg opacity-0 group-hover:opacity-100 blur-sm transition-all duration-300"></div>
                                <div className="relative p-1.5 sm:p-2 rounded-lg group-hover:shadow-md transition-all duration-300">
                                    <svg width="28" height="28" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg" className="sm:w-8 sm:h-8 transform group-hover:scale-110 transition-transform duration-300">
                            ...
                            {navItems.map((item) => (
                                <li key={item.id} className="relative group">
                                    {item.href ? (
                                        <a 
                                            href={item.href}
                                            className={`relative px-3 py-2 text-sm xl:text-base font-medium transition-all duration-300 ${
                                                activeSection === item.id 
                                                    ? 'text-blue-600' : !isOnTop ?'text-gray-700 hover:text-blue-600'
                                        ...
                                        >
                                            {item.label}
                                            {/* Active indicator */}
                                            <div className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-blue-500 to-cyan-400 transform transition-transform duration-300 ${
                                        ...
                                        <button 
                                            ...
                                            className={`relative px-3 py-2 text-sm xl:text-base font-medium transition-all duration-300 ${
                                                activeSection === item.id 
                                                    ? 'text-blue-600' :!isOnTop &&activeSection!=='home' ?'text-gray-700 hover:text-blue-600': !isOnTop ?'text-gray-700 hover:text-blue-600'
                                                    : 'text-black/70 hover:text-blue-600'
                                            }`}
                                        >
                                            {item.label}
                        ...
                        <div className="lg:hidden">
                            <button
                                ...
                                className="relative p-2 rounded-lg bg-gray-100 border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-all duration-300"
